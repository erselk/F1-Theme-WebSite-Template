...
export function SocialShare({ url, title, locale }: SocialShareProps) {
  ...
  const copyToClipboard = async () => {
    try {
      const textToCopy = `${shareMessages[locale]} ${url}`;
  ...
  // Social media icon colors for theme
  const iconColors = {
    x: isDark ? "hover:bg-black hover:border-black" : "hover:bg-white hover:border-black hover:text-black",
    facebook: isDark ? "hover:bg-[#1877F2] hover:border-[#1877F2]" : "hover:bg-[#1877F2] hover:text-white",
    linkedin: isDark ? "hover:bg-[#0A66C2] hover:border-[#0A66C2]" : "hover:bg-[#0A66C2] hover:text-white",
    whatsapp: isDark ? "hover:bg-[#25D366] hover:border-[#25D366]" : "hover:bg-[#25D366] hover:text-white",
    copy: isDark ? "hover:bg-electric-blue hover:border-electric-blue" : "hover:bg-electric-blue hover:text-white",
  };

  const buttonBaseClass = isDark 
    ? "bg-carbon-grey text-white rounded-full p-2 transition-all transform hover:scale-110 border border-transparent" 
    : "bg-light-grey/80 text-very-dark-grey rounded-full p-2 transition-all transform hover:scale-110 border border-transparent";
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-sm">
      ...
      {/* X Share */}
      <button 
        className={`${buttonBaseClass} ${iconColors.x}`}
      ...
      {/* Facebook Share */}
      <button 
        className={`${buttonBaseClass} ${iconColors.facebook}`} 
      ...
      {/* LinkedIn Share */}
      <button 
        className={`${buttonBaseClass} ${iconColors.linkedin}`}
      ...
      {/* WhatsApp Share */}
      <button 
        className={`${buttonBaseClass} ${iconColors.whatsapp}`}
      ...
      {/* Copy Link Button */}
      <button 
        className={`${buttonBaseClass} ${iconColors.copy}`}
      ...
      >
        ...
        {showCopySuccess && (
          <span className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-neon-green text-black text-xs py-1 px-2 rounded whitespace-nowrap">
